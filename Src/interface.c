...
void CubeHAL()
{
	...
	if(!(7-g))  GPIO_SetBits(GPIOA, GPIO_Pin_15);
	else GPIO_SetBits(GPIOB, (1<<(9-g)));
...
*******************************************************************************/
void CubeReset(){ for(uint8_t i=0; i<64; i++)	image[i] = 0; }
...
*******************************************************************************/
void CubeSet(){ for(uint8_t i=0; i<64; i++)	image[i] = 0xFF; }
...
void CubeSetPlane( uint8_t plane, uint8_t number )
{
	...
	if(plane == YPLANE)
	{
		for(uint8_t i=0; i<8; i++) image[8*i+7-number] = 0xFF;
	}

	if(plane == ZPLANE)
	{
		for(uint8_t i=0; i<8; i++) image[8*number+i] = 0xFF;
	}
}
...
void CubeResetPlane( uint8_t plane, uint8_t number )
{
	...
	if(plane == YPLANE)
	{
		for(uint8_t i=0; i<8; i++)	image[8*i+7-number] = 0;
	}

	if(plane == ZPLANE)
	{
		for(uint8_t i=0; i<8; i++)	image[8*number+i] = 0;
	}
}
...
void CubeSetLine( uint8_t plane, uint8_t x, uint8_t y )
{
	if(plane == XPLANE)
	{
		for(uint8_t i=0; i<8; i++)	image[8*y+7-x] |= (1<<i);
	}

	if(plane == YPLANE)
	{
		for(uint8_t i=0; i<8; i++)	image[8*y+i] |= (1<<(x));
	}

	if(plane == ZPLANE)
	{
		for(uint8_t i=0; i<8; i++)	image[8*i+y] |= (1<<(x));
	}
}
...
void ButtonsRead( uint8_t *pButtons )
{
	...
	/** If the n-button was pressed more than 2 scan cycles
		the corresponding button flag is only set
...
void MatrixShift()
{
	...
	//End reached?
	if( ((++matrPos/*+7*/)/4) >= strlen(pText2) ) {flagShift = false; matrPos = 0;}
