...
void SysTick_Handler(void)
{
	if(flagShift && frameTime) frameTime--;
	if(flagShift && !frameTime) MatrixShift();

	if(flagCubeShift && frameCubeTime) frameCubeTime--;
	if(flagCubeShift && !frameCubeTime) DrawStringShift();
	if(modesCounter) modesCounter--;

	if(bDeviceState == CONFIGURED) CUBECheckState();
	if(DebugDecrease < 32000) DebugDecrease++;
}
	...
	while(1)
	{
		...
		if((USBmode == STANDALONE) || (USBmode == PC_MONITORING))
		{
			...
			else
			{
				...
				CubeReset();
				if(!flagCubeShift) (*modeArray[modeCnt])();
			}
		}
	}
}
...
void TIM2_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
	{
		...
		if(++ItCounter >= 8)
		{
			...
			if(buttons & (1 << 0)) NVIC_SystemReset();
			if(buttons & (1 << 1)) DrawString("A B C D E F G H I", STRING_TWO_FACES);
			if(buttons & (1 << 2))
			{
				GOLOrNot ^= 1;
				BitWrite(GOLOrNot, Matrix[3], 4);
